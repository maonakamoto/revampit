...
**Pourquoi c'est bon pour la planète.** Chacun de ces usages, c'est un appareil neuf qu'on n'achète jamais : une vieille tablette en panneau mural remplace un écran domotique acheté ; un écran de recettes remplace un nouveau gadget de cuisine. Le calcul du carbone ci-dessus s'applique directement — environ deux tiers de l'empreinte d'un appareil sont figés dans sa fabrication, et on les évite entièrement lorsqu'une tablette existante reprend du service. Dans le même temps, un appareil qui dormirait sinon dans un tiroir ou finirait à la poubelle reste utile pendant des années — et hors de ce flux de déchets de 77,7 pour cent. Peu d'efforts, double gain : aucun appareil neuf et aucun déchet électronique.

**Comment en fabriquer un — et le verrouiller en mode kiosque.** Le chemin est court. Choisis le rôle et place la tablette là où il y a du courant — la plupart restent branchées en permanence, si bien qu'une batterie fatiguée n'a plus d'importance. Éventuellement, installe un système d'exploitation libre et allégé (section suivante), surtout s'il se connecte à internet, et configure l'unique application qu'il affichera désormais. L'étape décisive consiste à verrouiller l'appareil sur exactement cette application, pour que personne ne se promène dans les menus ou n'ouvre autre chose par accident — c'est ce qu'on appelle le **mode kiosque**. Android intègre l'*épinglage d'écran* ; pour les panneaux montés à demeure, il existe des applications kiosque dédiées qui épinglent l'appareil de manière encore plus stricte à un unique tableau de bord web ou à une seule application. Sur l'iPad, la même fonction s'appelle *Accès guidé* (Réglages → Accessibilité) — exactement ce qu'il faut pour l'appareil des enfants ou la tablette bloquée à sa dernière version d'iPadOS évoquée plus haut. Enfin, place l'appareil sur un Wi-Fi invité ou IoT et désactive tous les comptes et capteurs dont il n'a pas besoin. Le résultat : un terminal qui fait exactement une seule chose, pendant des années.

**Qui en veut un.** Plus de monde qu'on ne le croit. Les foyers : panneau domotique, appareil d'apprentissage pour enfant, écran de recettes ou de photos. Les cafés, commerces et cabinets : tableaux de menus et d'information (affichage dynamique, *digital signage*), bornes d'accueil et de retour, écrans de file d'attente et de réservation. Les écoles, bibliothèques et makerspaces : postes d'apprentissage et d'information dédiés et bon marché. Le soin et l'accessibilité : un appareil verrouillé sur une seule application d'appel vidéo ou de photos est souvent plus utilisable pour les personnes âgées ou celles qui ont besoin d'accompagnement qu'une tablette entièrement ouverte. Pour tous ces cas, une tablette reconditionnée en mode kiosque est bien moins chère — et bien plus écologique — qu'un matériel de borne conçu sur mesure. Ce sont précisément les appareils que RevampIT peut reconditionner à partir de dons et transmettre.

**Peut-on faire du tout open source ?** Sous Android : oui, de bout en bout. La base est un système d'exploitation libre comme LineageOS ou /e/OS (section suivante), les applications viennent du magasin open source *F-Droid*, le verrouillage est assuré par une application kiosque open source (comme *Webview Kiosk* ou *WallPanel*), et un tableau de bord est servi par le tout aussi open source *Home Assistant*. Aucun compte Google, aucun pistage, aucun frais de licence — idéal pour les associations, les écoles et quiconque veut un contrôle total. Seul l'iPad reste sur la touche : son bootloader verrouillé n'autorise aucun système d'exploitation libre, alors on s'y contente de l'*Accès guidé* sur l'iPadOS d'origine.
