...
export function MegaMenuContent({
  ...
  return (
    <div
      className={cn(
        "mt-2 bg-white rounded-2xl shadow-2xl shadow-neutral-200/50",
...
function MultiColumnLayout({
  ...
  return (
    <div className={cn("grid divide-x divide-neutral-100", gridCols)}>
      {groups.map((group, idx) => (
        <div key={idx} className="p-6">
          {group.section && (
            <Heading level={3} className="text-xs font-semibold text-neutral-600 uppercase tracking-wider mb-4">
        ...
        >
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <span className="font-medium text-neutral-900 group-hover:text-emerald-600 transition-colors">
                {getLabel(subItem, t)}
              </span>
          ...
          <ArrowRight className="w-4 h-4 text-neutral-300 group-hover:text-emerald-500 group-hover:translate-x-0.5 transition-all opacity-0 group-hover:opacity-100" />
        </Link>
      ))}
    </div>
  )
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {item.external && <ExternalLink className="w-3 h-3 text-neutral-600" />}
        </div>
      ...
      <ArrowRight className="w-4 h-4 text-neutral-300 group-hover:text-emerald-500 group-hover:translate-x-0.5 transition-all opacity-0 group-hover:opacity-100 mt-1 flex-shrink-0" />
    </Link>
  )
}
