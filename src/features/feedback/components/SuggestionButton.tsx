...
import { useSuggestionContext } from '../contexts/FeedbackContext'
import Link from 'next/link'
import { uiEvents } from '@/lib/ui/uiEvents'

// Import modular components and hooks
import { SuggestionTextarea, FeedbackScopeSelector, QuickSuggestions } from '.'
import { useSuggestionForm, useElementSelection } from '../hooks'
import { FeedbackScope } from '../types/components'
...
export default function SuggestionButton() {
  ...
  // Handle focus management when panel opens - Simplified
  // useEffect(() => {
  ...
  // Reset form when page changes or panel closes - Simplified
  // useEffect(() => {
  //   if (!isExpanded) {
  //     resetForm()
  //     setSubmitted(false)
  ...
  // Close when chat opens - Simplified
  // useEffect(() => {
  //   const offOpenChat = uiEvents.on('openChat', () => setIsExpanded(false))
  //   const offCloseSuggestion = uiEvents.on('closeSuggestion', () => setIsExpanded(false))
  //   return () => {
  //     offOpenChat()
  //     offCloseSuggestion()
  //   }
  ...
  return (
    <>
      ...
      {isElementSelectionMode && (
        ...
        >
          <div className="absolute top-16 left-4 bg-blue-600 text-white px-4 py-3 rounded-lg shadow-lg text-sm font-medium">
      ...
      {!isExpanded && !isElementSelectionMode && isVisible && (
        <div className="fixed right-4 bottom-24 z-[400]">
          <button
      ...
      {/* Backdrop - hidden during element selection mode */}
      {isExpanded && !isElementSelectionMode && (
        <div
          className="fixed inset-0 bg-black/30 z-[450] cursor-pointer"
          onClick={closePanelAndReset}
        />
      )}
      ...
      {/* Side Panel */}
      {isExpanded && (
        <div className="fixed right-0 top-1/2 -translate-y-1/2 z-[460] pointer-events-auto">
          <div
            ref={panelRef}
            role="dialog"
            aria-modal="true"
