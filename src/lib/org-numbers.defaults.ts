...
export const ORG_NUMBERS_DEFAULTS: Record<string, OrgNumber> = {
  ...
  co2_savings_per_device: d(
    'co2_savings_per_device', '150', 150,
    'kg CO₂e Einsparung pro aufbereitetem Laptop (konservativ)',
    'impact', 'medium',
    {
      methodology: 'ADEME-Kategorienmodell (SSOT: src/config/co2-impact.ts): Produktion+Distribution eines durchschnittlichen Neugeräts minus 15% Aufbereitungsaufwand, abgerundet auf 5 kg. Konsistent mit Fraunhofer/refurbed (69–91% Ersparnis, ⌀80%, ISO 14040/44) und ZHAW-LCA (vorläufig −82.7%).',
      calculation: 'Notebook (ADEME/ARCEP 2025): 182.3 kg Produktion+Distribution × 0.85 = 154.9 → konservativ 150 kg CO₂e.',
      sourceDocument: 'ADEME / ARCEP — Base Empreinte, Studie 2025 (offene Daten)',
      externalLink: 'https://impactco2.fr/outils/numerique',
  ...
  co2_factor_per_kg_device: d(
    ...
    'kg CO₂e pro kg Gerätegewicht (VERALTET — nicht mehr verwendet)',
    'impact', 'estimated',
    {
      methodology: 'VERALTET seit 2026-07: CO₂ wird nicht mehr aus dem Gewicht abgeleitet (physikalisch nicht haltbar über Gerätekassen hinweg). Ersetzt durch das ADEME-Kategorienmodell in src/config/co2-impact.ts — siehe /transparenz/co2.',
  ...
  annual_co2_saved_tons: d(
    'annual_co2_saved_tons', '~22', 22,
    'Tonnen CO₂ eingespart pro Jahr',
    'impact', 'estimated',
    {
      methodology: 'Hochrechnung aus verkauften Geräten × durchschnittlicher Einsparung pro Gerät (ADEME-Kategorienmodell, konservativ).',
      calculation: '~150 Geräte/Jahr × 150 kg = ~22.500 kg ≈ 22 t.',
