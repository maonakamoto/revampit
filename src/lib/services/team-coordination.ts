...
// ---- Progress helper (shared by UI) -----------------------------------------
// Returns a 0–100 fraction toward target, honouring direction. Null when a
// side is missing (nothing to chart). "Higher is better": current/target.
// "Lower is better": target/current (so beating the target caps at 100%).
