...
export interface NewListingQuestionData extends MarketplaceEmailData {
  askerName: string;
  listingTitle: string;
...
export function newListingQuestion(data: NewListingQuestionData): { subject: string; html: string; text: string } {
  return {
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Neue Frage zu deinem Inserat</h2>
        <p>Hallo ${escapeHtml(data.recipientName)},</p>
        <p><strong>${escapeHtml(data.askerName)}</strong> hat eine öffentliche Frage zu <strong>"${escapeHtml(data.listingTitle)}"</strong> gestellt:</p>
        <div style="background-color: #f3f4f6; border-radius: 8px; padding: 16px; margin: 16px 0;">
          <p style="margin: 0; color: #374151;">${escapeHtml(data.questionPreview)}</p>
        </div>
        <p>
          <a href="${data.listingUrl}" style="display: inline-block; background-color: #16a34a; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600;">
            Jetzt antworten
        ...
        <p style="color: #6b7280; font-size: 14px;">Deine Antwort wird öffentlich unter dem Inserat angezeigt.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
        <p style="color: #9ca3af; font-size: 12px;">${ORG.name} Marketplace</p>
      </div>
    `,
