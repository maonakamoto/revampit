      ...
      <div className="max-w-3xl mx-auto text-center">
        ...
        <div className="grid sm:grid-cols-3 gap-4 text-left">
          {[
            ...
            <div key={p.phase} className={`rounded-lg border p-4 ${p.done ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800/30' : 'bg-neutral-50 border-neutral-200'}`}>
              <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide mb-1">{p.phase}</div>
              <div className={`font-semibold mb-1 ${p.done ? 'text-primary-800 dark:text-primary-300' : 'text-neutral-700'}`}>{p.label}</div>
