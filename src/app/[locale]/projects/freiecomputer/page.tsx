...
type PageMessages = {
  ...
  mission: { title: string; cards: RawCard[] }
  history: { title: string; cards: RawCard[] }
  cta: { title: string; actions: RawAction[] }
}

export async function generateMetadata() {
  const t = await getTranslations('projects')
  const p = t.raw('freiecomputer') as PageMessages
  return { title: p.meta.title, description: p.meta.description }
}

export default async function FreieComputerPage() {
  const t = await getTranslations('projects')
  const p = t.raw('freiecomputer') as PageMessages
  ...
  const config: ProjectPageConfig = {
    ...
    sections: [
      ...
      },
      {
        ...
        layout: 'grid-2',
        cards: p.mission.cards.map(c => ({ title: c.title, description: c.description ?? '', features: c.features })),
      },
      {
        title: p.history.title,
        backgroundColor: 'white',
        layout: 'single',
        cards: p.history.cards.map(c => ({ title: c.title, description: c.description ?? '', features: c.features })),
      },
    ],
    ...
    <>
      ...
      <ProjectCallToAction
        ...
        actions={[
          ...
          { title: p.cta.actions[1].title, description: p.cta.actions[1].description, href: 'https://www.freiecomputer.ch', ctaText: p.cta.actions[1].cta },
