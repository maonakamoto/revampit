...
export default function PeerRepairDetailPage() {
  ...
  const fetchRequest = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/peer-repairs/requests/${id}`)
  ...
  const fetchOffers = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/peer-repairs/requests/${id}/offers`)
      const data = await response.json()
  ...
  const handleAcceptOffer = async (offerId: string) => {
    ...
    try {
      const response = await fetch(`/api/peer-repairs/requests/${id}/offers/${offerId}/accept`, {
        method: 'POST',
      })
  ...
  const budgetConfig = getBudgetTypeById(request.budgetType)
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {budgetConfig?.name || request.budgetType}
                    </p>
                    {request.budgetAmountCents && (
                      <p className="text-emerald-600 font-semibold">
                        CHF {(request.budgetAmountCents / 100).toFixed(0)}
                      </p>
                    )}
                  </div>
                </div>
