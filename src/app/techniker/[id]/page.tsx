...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!res.ok) return { title: 'Techniker | RevampIT' }
    const data = await res.json()
    const t: Technician = data.data?.technician
    if (!t) return { title: 'Techniker | RevampIT' }
    return {
      title: `${t.name} – Techniker | RevampIT`,
      description: t.bio ?? `${t.name} ist ${t.profileTier === 'professional' ? 'professioneller' : 'Community-'} Techniker bei RevampIT.`,
    }
  } catch {
    return { title: 'Techniker | RevampIT' }
  }
}
