...
function calculateMatchScore(
  ...
  // Skills overlap (most important) - 20 points per matching skill
  ...
  if (matchingSkills.length > 0) {
    const skillScore = matchingSkills.length * 20
    score += skillScore
    reasons.push(`${matchingSkills.length} passende Fähigkeiten`)
  }

  // Same canton - 15 points
  ...
  const isBudgetCompatible =
    ...
    helper.accepts_gratis // Helper willing to work for free
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const requestResult = await query(`
      SELECT
        id, requester_id, skills_needed, canton,
    ...
    // We'll get all active helpers who either:
    // 1. Have at least one matching skill, OR
    // 2. Are in the same canton
