 ...
 *
 * Tests for PUT /api/admin/repairer-applications/[id]/reject
 *
 * Behaviors locked:
...
jest.mock('@/lib/constants', () => ({
  SUPPORT_EMAIL: 'support@revamp-it.ch',
}))
...
function makeRequest(body: Record<string, unknown> = {}) {
  return new NextRequest('http://localhost/api/admin/repairer-applications/app-1/reject', {
    method: 'PUT',
...
describe('PUT /api/admin/repairer-applications/[id]/reject — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    mockAuth.mockResolvedValueOnce(null)
    const response = await PUT(makeRequest({ rejectionReason: 'Not qualified' }), makeContext())
    expect(response.status).toBe(401)
  })
})

describe('PUT /api/admin/repairer-applications/[id]/reject — validation', () => {
...
describe('PUT /api/admin/repairer-applications/[id]/reject — success', () => {
