 ...
 *
 * Tests for GET/PUT/DELETE /api/admin/team/profiles/[id]
 *
 * Behaviors locked:
 *   GET /api/admin/team/profiles/[id]
 *   - returns 401 when not authenticated
 *   - returns 404 when not found
 *   - returns 200 with profile
 *
...
const mockValidateUpdateTeamProfile = jest.fn()
...
jest.mock('@/db/schema', () => ({
  ...
  users: { id: 'u_id', name: 'u_name', email: 'u_email', createdAt: 'u_createdAt' },
}))
...
jest.mock('@/lib/schemas/team', () => ({
  validateUpdateTeamProfile: (...args: unknown[]) => mockValidateUpdateTeamProfile.apply(null, args),
}))
...
const MOCK_PROFILE = {
  id: 'prof-1', user_id: 'u-1', userId: 'u-1', user_name: 'Hans', user_email: 'hans@example.com',
  userEmail: 'hans@example.com', position: 'Developer', department: 'tech', is_active: true,
  created_at: '2026-01-01', updated_at: '2026-01-01',
}

function makeRequest(method = 'GET', body?: Record<string, unknown>) {
  return new NextRequest('http://localhost/api/admin/team/profiles/prof-1', {
    method,
...
beforeEach(() => {
  ...
  mockWhere.mockResolvedValue([MOCK_PROFILE])

  mockSet.mockReturnValue({ where: mockUpdateWhere })
  mockUpdateWhere.mockResolvedValue(undefined)
  mockDeleteWhere.mockResolvedValue(undefined)

  mockValidateUpdateTeamProfile.mockReturnValue({ success: true, data: { position: 'Senior Developer' } })
})
...
describe('GET /api/admin/team/profiles/[id] — unauthenticated', () => {
...
describe('GET /api/admin/team/profiles/[id] — authenticated', () => {
  ...
  it('returns 200 with profile', async () => {
    ...
    expect(body.data.id).toBe('prof-1')
  })
})
...
describe('PUT /api/admin/team/profiles/[id] — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    mockAuth.mockResolvedValueOnce(null)
    const response = await PUT(makeRequest('PUT', { position: 'Senior Developer' }), makeContext())
    expect(response.status).toBe(401)
  })
})

describe('PUT /api/admin/team/profiles/[id] — validation', () => {
  it('returns 400 when body is invalid', async () => {
    mockValidateUpdateTeamProfile.mockReturnValueOnce({
      success: false,
  ...
  it('returns 400 when no fields to update', async () => {
    mockValidateUpdateTeamProfile.mockReturnValueOnce({ success: true, data: {} })
    const response = await PUT(makeRequest('PUT', {}), makeContext())
    expect(response.status).toBe(400)
  })
...
describe('PUT /api/admin/team/profiles/[id] — success', () => {
...
describe('DELETE /api/admin/team/profiles/[id] — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    mockAuth.mockResolvedValueOnce(null)
...
describe('DELETE /api/admin/team/profiles/[id] — authenticated', () => {
  it('returns 404 when not found', async () => {
    mockWhere.mockResolvedValueOnce([])
    const response = await DELETE(makeRequest('DELETE'), makeContext())
    expect(response.status).toBe(404)
