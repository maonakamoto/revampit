...
import { db } from '@/db'
import { listings, listingReports, marketplaceOrders, users } from '@/db/schema'
  ...
  try {
    ...
    const [row] = await db
      .select({
        ...
        revampit: sql<number>`count(*) FILTER (WHERE ${revampitSellerCondition})`,
        community: sql<number>`count(*) FILTER (WHERE NOT ${revampitSellerCondition})`,
