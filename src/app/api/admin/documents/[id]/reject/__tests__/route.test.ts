 ...
 *
 * Tests for PUT /api/admin/documents/[id]/reject
 *
 * Mission-relevant: rejectionReason is required and guards against rejecting
 * already-approved or already-rejected documents.
 *
 * Behaviors locked:
 ...
 *   - returns 400 when document already rejected
...
jest.mock('@/config/org', () => ({
  CONTACT: { supportEmail: 'support@revamp-it.ch' },
}))
...
function makeRequest(body: Record<string, unknown> = { rejectionReason: 'Unleserliches Dokument' }) {
  return new NextRequest('http://localhost/api/admin/documents/doc-1/reject', {
    method: 'PUT',
...
describe('PUT /api/admin/documents/[id]/reject — unauthenticated', () => {
...
describe('PUT /api/admin/documents/[id]/reject — validation', () => {
...
describe('PUT /api/admin/documents/[id]/reject — service errors', () => {
  ...
  it('returns 400 when document already rejected', async () => {
    mockDbExecute.mockReset()
    mockDbExecute.mockResolvedValueOnce({ rows: [{ ...MOCK_DOC_PENDING, status: 'rejected' }] })
    const response = await PUT(makeRequest(), makeContext())
    expect(response.status).toBe(400)
  })
})

describe('PUT /api/admin/documents/[id]/reject — success', () => {
