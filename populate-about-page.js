...
/**
 * Migration script to populate About page content in Strapi
 * Run this script to automatically migrate the hardcoded About page content to Strapi
 */
...
async function populateAboutContent() {
  try {
    ...
    // First, let's try to create the content using fetch
    const response = await fetch('http://localhost:1337/api/static-pages', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      console.log('🔗 You can now view it at: http://localhost:1337/about');
      return result;
    } else {
      const error = await response.text();
      console.error('❌ Failed to create content:', error);

      // If it fails due to auth, try to get an API token
      if (response.status === 401 || response.status === 403) {
        console.log('🔐 Authentication required. Please create an API token in Strapi admin.');
        console.log('📋 Go to: http://localhost:1337/admin');
        console.log('📋 Settings > API Tokens > Create new token');
        console.log('📋 Give it full permissions for Static Pages');
        console.log('📋 Then run this script again with STRAPI_API_TOKEN environment variable');
      }
    }
  } catch (error) {
    console.error('❌ Error:', error.message);

    // Fallback: Create a JSON file that can be imported manually
    console.log('📝 Creating content file for manual import...');
    const fs = require('fs');
    fs.writeFileSync('about-content-import.json', JSON.stringify(content, null, 2));
    console.log('✅ Content saved to: about-content-import.json');
    console.log('📋 To import manually:');
    console.log('   1. Go to Strapi admin: http://localhost:1337/admin');
    console.log('   2. Content Manager > Static Pages > Create new entry');
    console.log('   3. Copy the content from the JSON file');
  }
}
