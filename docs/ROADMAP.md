...
2. **Zeiterfassung de-frankenstein** — audit done; design tokens were actually clean (frankenstein is in the LOGIC).
   ...
   - **Remaining (careful, payroll-adjacent — do attended):** the shift-clock persist path (`addShiftEntry` rebuilds its own PUT); the ~900-line `useTimecardDraft` god-hook needs splitting; two Save/Submit clusters with duplicated disabled logic → derive `canSave`/`canSubmit` once. Lower-severity: day-grid re-quantizes non-30-min breaks (edge case; exact times survive in the Von/Bis fields — only a grid gesture re-quantizes). **NOTE: the audit's "dead week code" is NOT dead** — `buildTimecardEntriesFromSchedule`/`startOfWeek`/`period_type='week'` are live server-side for historical week timecards; do not remove.

2b. **Geräte-Eingang / product capture** — ✅ **Unified 2026-07-17.** `/admin/intake/capture` is the canonical capture page; legacy `/admin/erfassung` renders the same component. Text, photo, CSV/Excel and speech are input channels into one product schema, followed by review and one destination decision. Deleted the quick/physical mode toggle and duplicate quick form. Quality is the recommended path; inventory, parts and recycling are explicit alternatives. Untested shop publication is a price- and reason-gated audited exception and never receives verification. The pipeline, QR labels, buyer-visible checks, two-person QA, page guide/Hirn context and public open process are shipped. **Remaining integration cleanup:** the legacy `/api/admin/erfassung` write endpoint and Kivvi-only `/api/inventory/import-csv` remain for external clients; retire them only after caller telemetry confirms zero use.
